...
class Acl extends ZendAcl {
    ...
    public function __construct($config) {
        if (!isset($config['likerow_server']['acl']['roles']) || !isset($config['likerow_server']['acl']['resources'])) {
            throw new \Exception('Invalid ACL Config found');
        }

        $roles = $config['likerow_server']['acl']['roles'];
        if (!isset($roles[self::DEFAULT_ROLE])) {
            $roles[self::DEFAULT_ROLE] = '';
        }

        $this->_addRoles($roles)
                ->_addResources($config['likerow_server']['acl']['resources']);
    }
